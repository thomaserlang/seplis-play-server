...
from seplis_play_server import config, logger, utils
...
class Play_scan:
    ...
    async def get_metadata(self, path):
        ...
        if not data:
            raise Exception(
                f'Failed to get metadata from {path}, either this is not a media file or it is corrupt.'
            )
    ...
    async def get_keyframes(self, path):
        ...
        if not data:
            logger.error(
                f'Failed to get keyframes from {path}, either this is not a media file or it is corrupt.'
            )
            return
