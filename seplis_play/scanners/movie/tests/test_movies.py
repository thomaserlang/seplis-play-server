...
@pytest.mark.asyncio
async def test_movie_parse() -> None:
    from seplis_play.scanners import MovieScan

    scanner = MovieScan('/')
    assert (
        scanner.parse('Uncharted (2160p BluRay x265 10bit HDR Tigole).mkv') == 'Uncharted'
    )
    assert (
        scanner.parse(
    ...
    assert (
        scanner.parse(
            ...
            '[Bluray-1080p Proper][EAC3 5.1][x265]-GalaxyRG265.mkv'
        )
        == 'Justice League Crisis on Infinite Earths Part Two (2024)'
    )
